...
class Parser:
    ...
    def program(self):
        print("PROGRAM")
        x = SymbolTable(self.curToken.value, self.curToken.kind.name, self.curToken.kind.value, 'none', 'none', 'global')
    ...
    def primary(self):
        ...
        if self.checkToken(TokenType.NUMBER):
            if SymbolTable.isEmptyStack() != 0:
              y = SymbolTable.pop()
              y.modify(self.curToken)
            self.next()
